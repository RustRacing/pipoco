...
impl KvStore for FailingKv {
    fn read(&mut self, _key: &[u8], _out: &mut [u8]) -> Result<usize, KvError> { Err(KvError::NotFound) }
    fn write(&mut self, _key: &[u8], _data: &[u8]) -> Result<(), KvError> { Err(KvError::Io) }
}
...
fn try_load_with_kv_failure_keeps_defaults() {
    ...
    state.inj_angle_btdc_x10[0] = 111;
    state.cam_missing_timeout_ms = 250;
    ...
    assert_eq!(state.inj_angle_btdc_x10[0], 111);
    assert_eq!(state.cam_missing_timeout_ms, 250);
}
...
fn factory_reset_restores_safe_defaults() {
    let mut state = EcuState::new();
    // Modify state
    state.ipw_table[0][0] = 9999;
    state.ignition_table[0][0] = 45;
    state.inj_angle_btdc_x10[0] = 99;
    state.cam_missing_timeout_ms = 250;
    let mut store = PersistedEcuPageStore::new(&mut state, FailingKv);
    store.factory_reset();
    ...
    let n1 = store.read_page(ecu_core::ts::pages::PAGE_FUEL, &mut fuel).unwrap();
    let n2 = store.read_page(ecu_core::ts::pages::PAGE_IGN, &mut ign).unwrap();
    let n3 = store.read_page(ecu_core::ts::pages::PAGE_ANGLES, &mut angles).unwrap();
    assert_eq!(n1, 512);
    assert_eq!(n2, 512);
    assert_eq!(n3, 68);
