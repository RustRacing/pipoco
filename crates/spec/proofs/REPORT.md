...
## 12. QEMU Soak Roadmap and Evidence (Current)

### Phase Status

- Phase 5 (opaque-firmware soak): Completed.
  - Live firmware telemetry remains unavailable in this slice because Speeduino telemetry is external/opaque.
  - Live TS map upload remains unavailable because transport for live map injection is not yet established in this environment.
- Phase 6 (host-runtime telemetry proof under QEMU soak): In progress.
  - New evidence source: `host_telemetry.json` generated by the repo-controlled ATmega2560 mirror runtime.
  - Host runtime telemetry and workload counters are now collected in QEMU soaks and surfaced in `report.md`.
  - Firmware telemetry status is explicitly tracked as unavailable until a firmware telemetry channel is exposed.
