...
pub fn baro_from_counts(adc_counts: u16) -> Kpa10 {
    ...
    Kpa10::new((BARO_MIN_KPA10 as u32 + scaled) as u16)
}
...
mod tests {
    ...
    fn clamps_at_adc_endpoints() {
        assert_eq!(baro_from_counts(0), Kpa10::new(500));
        assert_eq!(baro_from_counts(4095), Kpa10::new(1200));
    }

    #[test]
    fn midpoint_uses_floor_linear_interpolation() {
        // floor((2048 * (1200 - 500)) / 4095) + 500 = 850
        assert_eq!(baro_from_counts(2048), Kpa10::new(850));
    }

    #[test]
    fn is_monotonic_non_decreasing_over_full_adc_domain() {
        let mut prev = baro_from_counts(0).get();
        let mut adc = 1u16;
        while adc <= 4095 {
            let current = baro_from_counts(adc).get();
            assert!(
                current >= prev,
                "adc={} current={} prev={}",
                adc,
