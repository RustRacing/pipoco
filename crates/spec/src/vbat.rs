...
pub fn vbat_correction(vbat_corr_curve: &Curve16, vbat_mv: Millivolts) -> RatioX1000 {
    RatioX1000::new(lookup_curve_u16(vbat_corr_curve, vbat_mv.get()))
}
...
mod tests {
    ...
    fn vbat_low_edge_clamps_to_min_axis() {
        let corr = vbat_correction(&vbat_curve(), Millivolts::new(7000));
        assert_eq!(corr, RatioX1000::new(1200));
    }

    #[test]
    fn vbat_nominal_point_matches_curve_cell() {
        let corr = vbat_correction(&vbat_curve(), Millivolts::new(12_000));
        assert_eq!(corr, RatioX1000::new(1000));
    }

    #[test]
    fn vbat_high_edge_clamps_to_max_axis() {
        let corr = vbat_correction(&vbat_curve(), Millivolts::new(17_000));
        assert_eq!(corr, RatioX1000::new(900));
    }
}
