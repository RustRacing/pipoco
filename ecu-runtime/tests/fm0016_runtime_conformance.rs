...
use ecu_runtime::{
    ...
    RuntimeConformanceStatus, RuntimeObservedSurface, RuntimeSemanticAfrOverride,
    ...
    RuntimeSemanticScheduleCalibration, RuntimeSemanticScheduleDiagnostic,
    RuntimeSemanticScheduleEvent, RuntimeSemanticScheduleEventKind, RuntimeSemanticState,
    RuntimeSemanticTable2dI16, RuntimeSemanticTable2dU16, RuntimeSemanticTable2dU32,
    RuntimeSemanticTorqueInput, StepInputs, TorqueInputs,
};
use ecu_spec::{
...
fn to_semantic_input(input: &InputSnapshot) -> RuntimeSemanticInputSnapshot {
    ...
    RuntimeSemanticInputSnapshot {
        ...
        sync: match input.sync {
            ecu_spec::SyncState::Synced => DomainSyncState::Synced,
...
fn semantic_schedule_authority(input: RuntimeSemanticInputSnapshot) -> EngineTimeAuthority {
    if input.sync == DomainSyncState::Synced {
...
fn fuel_cut_from_actions(actions: &ActionBatch<12>) -> bool {
...
fn spark_cut_from_actions(actions: &ActionBatch<12>) -> bool {
    for action in (*actions).iter() {
        if let Action::CancelScheduler(_) = action {
            return true;
        }
...
fn runtime_semantic_schedule_requires_validated_authority_not_sync_alone() {
    ...
    assert_eq!(semantic_input.sync, DomainSyncState::Synced);
...
fn schedule_conformance_test(case: &fm0016_fixture_matrix::FixtureCase) {
    ...
    if !matches!(semantic_input.sync, DomainSyncState::Synced) {
        assert_eq!(
