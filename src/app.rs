...
use crate::{Channel, EcuState, Scheduler, TriggerDecoder};
...
impl<T: TimeSource> EcuApp<T> {
    ...
    pub fn on_timestamp(&mut self, ts_us: u32) {
        ...
        if self.decoder.synced() {
            self.state.rpm = self.decoder.rpm();
            self.state.synced = true;
            self.state.tooth_count = self.decoder.tooth();
    ...
    pub fn on_cam_edge(&mut self) {
        if self.decoder.synced() {
            ...
            if self.state.diag_cam.active {
                ...
                self.state.diag_log.push(crate::diag::DiagEvent {
                    code: crate::diag::DiagCode::CamMissing,
                    start_us: self.state.diag_cam.start_us,
                    end_us: now,
                });
    ...
    fn check_cam_diag(&mut self, now: u32) {
        if !self.config.has_cam { return; }
        let timeout_us = (self.state.cam_missing_timeout_ms as u32) * 1000;
        let missing = if self.last_cam_edge_us == 0 { now >= timeout_us } else { now.wrapping_sub(self.last_cam_edge_us) >= timeout_us };
        if missing && !self.state.diag_cam.active {
            self.state.diag_cam.active = true;
    ...
    fn schedule_injection(&mut self, now: u32) {
        ...
        let pw = self.state.calculate_fuel(self.state.rpm, load) as u32;
        ...
        match self.config.injection_mode {
            InjectionMode::Batch => {
                ...
                let start_ticks = now_ticks.wrapping_add(((dt0 as u64) * (hz as u64) / 1_000_000u64) as u32);
                ...
                for i in 0..(self.config.outputs.inj_count as usize) {
                    ...
                    if !self.scheduler.schedule_ticks(start_ticks, ch, true)
                        || !self.scheduler.schedule_ticks(start_ticks.wrapping_add(width_ticks), ch, false)
                    {
            ...
            InjectionMode::Sequential => {
                ...
                if self.config.has_cam && !self.cam_phase_known {
                    ...
                    for i in 0..(self.config.outputs.inj_count as usize) {
                        ...
                        if !self.scheduler.schedule_ticks(start_ticks, ch, true)
                            || !self.scheduler.schedule_ticks(start_ticks.wrapping_add(width_ticks), ch, false)
                        {
                            self.scheduler_full_count =
                                self.scheduler_full_count.saturating_add(1);
                        }
                    }
                    if self.scheduler_full_count > 8 {
                        self.latch.latch_off();
                ...
                let len = if self.inj_seq_len == 0 { 1 } else { self.inj_seq_len } as usize;
                let base = now.wrapping_add(INJECTION_DELAY_US);
                #[cfg(feature = "sched-simple")]
                {
                    ...
                    let ang0_x10 = self.state.inj_angle_btdc_x10[idx0] as u32;
                    ...
                    let ang1_x10 = self.state.inj_angle_btdc_x10[idx1] as u32;
                    let start1 = base
                    ...
                    let start0_ticks = now_ticks.wrapping_add(((dt0 as u64) * (hz as u64) / 1_000_000u64) as u32);
                    let start1_ticks = now_ticks.wrapping_add(((dt1 as u64) * (hz as u64) / 1_000_000u64) as u32);
                    let width_ticks = ((pw as u64) * (hz as u64) / 1_000_000u64) as u32;
                    let _ = self.scheduler.schedule_ticks(start0_ticks, ch0, true);
                    let _ = self.scheduler.schedule_ticks(start0_ticks.wrapping_add(width_ticks), ch0, false);
                    let _ = self.scheduler.schedule_ticks(start1_ticks, ch1, true);
                    let _ = self.scheduler.schedule_ticks(start1_ticks.wrapping_add(width_ticks), ch1, false);
                }
                #[cfg(not(feature = "sched-simple"))]
                {
                    // Angle-accurate path
                    let modulo = if self.config.has_cam && self.cam_phase_known { 7200u16 } else { 3600u16 };
                    // First cyl
                    let idx0 = (self.inj_idx as usize) % len;
                    let ch0 = self.inj_seq[idx0];
                    let inj0_btdc_x10 = self.state.inj_angle_btdc_x10[idx0];
                    let tdc0_x10 = self.state.tdc_per_cyl_x10[idx0];
                    let target0 = (tdc0_x10.wrapping_sub(inj0_btdc_x10).wrapping_add(self.state.tooth0_angle_x10)) % modulo;
                    let start0 = self.decoder.time_for_target_angle(base, target0, modulo).unwrap_or(base);
                    let ts = self.decoder.time_source();
                    let now_ticks = ts.ticks();
                    let hz = ts.freq_hz();
                    self.scheduler.deactivate_channel_all(ch0);
                    ...
                    let _ = self.scheduler.schedule_ticks(start0_ticks, ch0, true);
                    let _ = self.scheduler.schedule_ticks(start0_ticks.wrapping_add(width0_ticks), ch0, false);
                    // Second cyl
                    let idx1 = (self.inj_idx.wrapping_add(1) as usize) % len;
                    let ch1 = self.inj_seq[idx1];
                    let inj1_btdc_x10 = self.state.inj_angle_btdc_x10[idx1];
                    let tdc1_x10 = self.state.tdc_per_cyl_x10[idx1];
                    let target1 = (tdc1_x10.wrapping_sub(inj1_btdc_x10).wrapping_add(self.state.tooth0_angle_x10)) % modulo;
                    let start1 = self.decoder.time_for_target_angle(base, target1, modulo).unwrap_or(base);
                    self.scheduler.deactivate_channel_all(ch1);
                    let dt1 = start1.wrapping_sub(base);
                    let start1_ticks = now_ticks.wrapping_add(((dt1 as u64) * (hz as u64) / 1_000_000u64) as u32);
                    let width1_ticks = ((pw as u64) * (hz as u64) / 1_000_000u64) as u32;
                    let _ = self.scheduler.schedule_ticks(start1_ticks, ch1, true);
                    let _ = self.scheduler.schedule_ticks(start1_ticks.wrapping_add(width1_ticks), ch1, false);
                }
        ...
        match self.config.ignition_mode {
            IgnitionMode::Wasted => {
                ...
                for i in 0..(self.config.outputs.ign_count as usize).min(2) {
                    ...
                    if !self.scheduler.schedule(start, ch, true) {
                        self.scheduler_full_count = self.scheduler_full_count.saturating_add(1);
                    }
                    if !self.scheduler.schedule(end, ch, false) {
                        self.scheduler_full_count = self.scheduler_full_count.saturating_add(1);
                    }
                }
            }
            IgnitionMode::Sequential => {
                ...
                let len = if self.ign_seq_len == 0 { 1 } else { self.ign_seq_len } as usize;
                let idx = (self.ign_idx as usize) % len;
                let ch = self.ign_seq[idx];
                // Compute target spark angle = TDC(cyl) - advance
                let advance_deg = self.state.calculate_ignition_timing(self.state.rpm, DEFAULT_LOAD_KPA) as i32; // deg
                let advance_x10 = if advance_deg <= 0 { 0u16 } else { (advance_deg as u16) * 10 };
                let ts = self.decoder.time_source();
                let now_ticks = ts.ticks();
                let hz = ts.freq_hz();
                ...
                #[cfg(feature = "sched-simple")]
                {
                    let rpm = core::cmp::max(1u32, self.state.rpm as u32);
                    let rev_us = 60_000_000u32 / rpm;
                    let tdc_x10 = self.state.tdc_per_cyl_x10[idx];
                    let spark_ang_x10 = (tdc_x10.wrapping_sub(advance_x10).wrapping_add(self.state.tooth0_angle_x10)) % 3600;
                    let spark_time = now.wrapping_add((rev_us.saturating_mul(spark_ang_x10 as u32)) / 3600);
                    let dwell_us = self.state.calculate_dwell();
                    let mut dwell_start = spark_time.wrapping_sub(dwell_us);
                    if dwell_start < now { dwell_start = now; }
                    let start_ticks = now_ticks.wrapping_add(((dwell_start.wrapping_sub(now) as u64) * (hz as u64) / 1_000_000u64) as u32);
                    let spark_ticks = now_ticks.wrapping_add(((spark_time.wrapping_sub(now) as u64) * (hz as u64) / 1_000_000u64) as u32);
                ...
                #[cfg(not(feature = "sched-simple"))]
                {
                    let modulo = if self.config.has_cam && self.cam_phase_known { 7200u16 } else { 3600u16 };
                    let tdc_x10 = self.state.tdc_per_cyl_x10[idx];
                    let spark_ang_x10 = (tdc_x10.wrapping_sub(advance_x10).wrapping_add(self.state.tooth0_angle_x10)) % modulo;
                    let spark_time = self.decoder.time_for_target_angle(now, spark_ang_x10, modulo).unwrap_or(now);
                    let dwell_us = self.state.calculate_dwell();
                    let mut dwell_start = spark_time.wrapping_sub(dwell_us);
                    if dwell_start < now { dwell_start = now; }
                    let start_ticks = now_ticks.wrapping_add(((dwell_start.wrapping_sub(now) as u64) * (hz as u64) / 1_000_000u64) as u32);
...
mod tests {
    ...
    fn test_sequential_injection_reschedules_future() {
        ...
        let _initial: std::vec::Vec<u32> = s
            .events_mut()
            .iter()
            .filter(|e| e.is_active() && e.state() && (e.channel().as_u8() == Channel::INJ1.as_u8() || e.channel().as_u8() == Channel::INJ2.as_u8()))
            .map(|e| e.time())
            .collect();
    ...
    fn test_sequential_cam_fallback_degrades_to_batch() {
        ...
        for ch in 0..4u8 {
            let count = s
                ...
                .count();
            if count > 0 { channels_with_on += 1; }
        }
        assert_eq!(channels_with_on, 4, "should schedule all injectors in batch");
        app.scheduler = s;
    }
    ...
    fn test_sequential_720_coverage_window_per_cycle() {
        ...
        let cfg = EcuConfig {
            ...
            tdc_per_cyl_x10: {
                let mut a = [0u16; 16];
                a[0] = 0; a[1] = 1800; a[2] = 3600; a[3] = 5400; a
            },
            tooth0_angle_x10: 0,
        };
        let mut app = EcuApp::new_with_config(time, cfg);
        ...
        for e in s.events_mut().iter().filter(|e| e.is_active()) {
            ...
            if ch < 4 {
                if e.state() { ons[ch] += 1; } else { offs[ch] += 1; }
            }
        }
        for i in 0..4 { assert_eq!(ons[i], 1, "one ON per channel"); assert_eq!(offs[i], 1, "one OFF per channel"); }
        app.scheduler = s;
    }
    ...
    fn test_sequential_reschedule_stress_active_bound() {
        let time = MockTime;
        let mut inj = [Channel::from_index(0); 16];
        inj[0] = Channel::INJ1; inj[1] = Channel::INJ2; inj[2] = Channel::from_index(2); inj[3] = Channel::from_index(3);
        let ign = [Channel::from_index(0); 16];
        let outputs = OutputChannels { inj_channels: inj, inj_count: 4, ign_channels: ign, ign_count: 0 };
        let cfg = EcuConfig { cylinders: 4, firing_order: &[1,3,4,2], injection_mode: InjectionMode::Sequential, ignition_mode: IgnitionMode::Wasted, has_cam: false, outputs, inj_angle_btdc_x10: [0;16], tdc_per_cyl_x10: [0;16], tooth0_angle_x10: 0 };
        let mut app = EcuApp::new_with_config(time, cfg);
        // Ramp RPM and schedule repeatedly; ensure active events stay bounded
        for i in 0..500u32 {
            app.state.rpm = if i % 2 == 0 { 1200 } else { 6500 };
    ...
    fn test_sched_simple_reschedule_clears_future() {
        let time = MockTime;
        let mut inj = [Channel::from_index(0); 16];
        inj[0] = Channel::INJ1; inj[1] = Channel::INJ2;
        let ign = [Channel::from_index(0); 16];
        let outputs = OutputChannels { inj_channels: inj, inj_count: 2, ign_channels: ign, ign_count: 0 };
        let cfg = EcuConfig { cylinders: 2, firing_order: &[1,2], injection_mode: InjectionMode::Sequential, ignition_mode: IgnitionMode::Wasted, has_cam: false, outputs, inj_angle_btdc_x10: [0;16], tdc_per_cyl_x10: [0;16], tooth0_angle_x10: 0 };
        let mut app = EcuApp::new_with_config(time, cfg);
        app.state.rpm = 1500;
        app.schedule_injection(10_000);
        ...
        app.schedule_injection(20_000);
        let mut s2 = app.scheduler;
        let inj1_on = s2.events_mut().iter().filter(|e| e.is_active() && e.channel().as_u8()==Channel::INJ1.as_u8() && e.state()).count();
        let inj2_on = s2.events_mut().iter().filter(|e| e.is_active() && e.channel().as_u8()==Channel::INJ2.as_u8() && e.state()).count();
        assert_eq!(inj1_on, 1); assert_eq!(inj2_on, 1);
        app.scheduler = s2;
    }
    #[test]
    ...
    fn test_ignition_dwell_past_now_is_clamped() {
        ...
        let mut ign = [Channel::from_index(0); 16];
        ign[0] = Channel::IGN1;
        let outputs = OutputChannels { inj_channels: inj, inj_count: 0, ign_channels: ign, ign_count: 1 };
        let cfg = EcuConfig { cylinders: 1, firing_order: &[1], injection_mode: InjectionMode::Batch, ignition_mode: IgnitionMode::Sequential, has_cam: false, outputs, inj_angle_btdc_x10: [0;16], tdc_per_cyl_x10: [0;16], tooth0_angle_x10: 0 };
        let mut app = EcuApp::new_with_config(time, cfg);
        // Force large dwell by simulating low battery
        app.state.battery_voltage_mv = 5000; // max dwell
        // No decoder rev estimate so spark_time defaults to now; dwell_start would be < now without clamp
        app.schedule_ignition(10_000);
        let mut s = app.scheduler;
        // Collect IGN1 events
        let mut times: Vec<(bool, u32)> = s.events_mut().iter().filter(|e| e.is_active() && e.channel().as_u8()==Channel::IGN1.as_u8()).map(|e| (e.state(), e.time())).collect();
    ...
    fn test_sched_simple_injection_guard() {
        // Sanity guard: in sched-simple mode, sequential injection schedules
    ...
    fn test_cam_missing_diag_and_clear() {
        ...
        struct FixedTime;
        impl TimeSource for FixedTime { fn micros(&self) -> u32 { 600_000 } }
        let time = FixedTime;
        let inj = [Channel::from_index(0); 16];
        let ign = [Channel::from_index(0); 16];
        let outputs = OutputChannels { inj_channels: inj, inj_count: 0, ign_channels: ign, ign_count: 0 };
        let cfg = EcuConfig {
            cylinders: 1,
            firing_order: &[1],
            injection_mode: InjectionMode::Batch,
        ...
        let mut app = EcuApp::new_with_config(time, cfg);
        app.state.cam_missing_timeout_ms = 500;
        // Seed decoder sync (two edges with a gap)
        app.on_timestamp(1_000);
        app.on_timestamp(3_000);
        // No flag before timeout
        assert!(!app.state.diag_cam.active);
        // Directly invoke cam diagnostic check at time past timeout
        app.check_cam_diag(600_000);
        assert!(app.state.diag_cam.active, "cam missing should be active");
        // Cam edge clears and logs event
        app.on_cam_edge();
        assert!(!app.state.diag_cam.active, "cam missing cleared after edge");
        assert!(app.state.diag_log.events.iter().any(|e| matches!(e, Some(ev) if matches!(ev.code, crate::diag::DiagCode::CamMissing))),
