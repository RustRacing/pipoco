...
mod tests {
    ...
    fn one_revolution_at_1000_rpm() {
        ...
        let edges = core::array::from_fn::<_, 59, _>(|_| gen.next_edge().unwrap());

        // First edge at t=0
        assert_eq!(edges[0].at_us.get(), 0);
    ...
    fn missing_gap_is_three_slots() {
        ...
        // The configured missing slots remain the mechanical source of that gap.
        assert_eq!(p.missing_slots, 2);
    }
