...
impl<P: PlantProfile> ClosedLoopPlant<P> {
    ...
    fn apply_injector_transition(
        ...
        let idx = validate_channel_index(transition.channel)
            .map_err(|_| PlantError::UnknownInjectorChannel(transition.channel))?;
    ...
    fn apply_ignition_transition(
        ...
        let idx = validate_channel_index(transition.channel)
            .map_err(|_| PlantError::UnknownIgnitionChannel(transition.channel))?;
        self.validate_cylinder(cylinder)?;
        match transition.level {
            OutputLevel::High => {
...
fn validate_channel_index(channel: ChannelId) -> Result<usize, ()> {
    let idx = channel.get() as usize;
    if idx < PLANT_MAX_CHANNELS {
        Ok(idx)
    } else {
