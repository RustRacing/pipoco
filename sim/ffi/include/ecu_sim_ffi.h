...
typedef struct {
  ...
  uint16_t knock_x100;
  uint16_t vehicle_speed_kph10;
  uint16_t maf_x100;
  int16_t cam_phase_deg10;
  uint8_t cam_phase_valid;
  uint8_t validity_flags;
} ecu_sim_sensor_frame_t;

#define ECU_SIM_SENSOR_VALID_MAF ((uint8_t)(1u << 0))
#define ECU_SIM_SENSOR_VALID_KNOCK ((uint8_t)(1u << 1))
#define ECU_SIM_SENSOR_VALID_VEHICLE_SPEED ((uint8_t)(1u << 2))
#define ECU_SIM_SENSOR_VALID_LAMBDA ((uint8_t)(1u << 3))
...
size_t ecu_sim_handle_dequeue_events(ecu_sim_handle_t *handle,
                                     ...
                                     size_t cap);
ecu_sim_status_t ecu_sim_handle_dequeue_events_status(ecu_sim_handle_t *handle,
                                                      ecu_sim_output_event_t *out,
                                                      size_t cap,
                                                      size_t *copied_out);
...
ecu_sim_status_t ecu_sim_dequeue_events_status(ecu_sim_output_event_t *out,
                                               size_t cap,
                                               size_t *copied_out);
