...
use ecu_sim_core::{
    ...
    spark::SparkCommand,
    InitialPlantState, MisfireReason,
};
...
fn wall_film_changes_delivered_fuel_without_hiding_raw_command() {
    ...
    assert_eq!(
        plant.state().cylinders[0].last_injection_pw_us,
        Micros(3500)
    );
...
fn valve_event_ivc_sweep_changes_low_and_high_rpm_torque_shape() {
    ...
    assert!(
        early_low.state().cylinders[0].air_mass_ug.0 > late_low.state().cylinders[0].air_mass_ug.0
    );
    assert!(
        late_high.state().cylinders[0].air_mass_ug.0
            > early_high.state().cylinders[0].air_mass_ug.0
    );
    assert!(
