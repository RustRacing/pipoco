...
use ecu_domain::{
    AbsoluteTimeAuthority, Degrees10, EngineTimeAuthority, Micros, PhaseSyncState, SyncState, Ticks,
};
...
use ecu_trigger::{
    ...
    SecondaryTriggerMode, SecondaryTriggerProfile, SyncLossReason, TriggerAngleAuthority,
    TriggerDiagnostics, TriggerEdge, TriggerLevel, TriggerSpeed, TriggerValidationError,
    DEFAULT_MISSING_TOOTH_GAP_RATIO_X1000,
};

pub const M50_DRY_CRANK_FIXTURE_FORMAT_VERSION: u8 = 1;
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct M50DryCrankFixturePlaceholder {
    pub format_version: u8,
    pub name: &'static str,
    pub profile_name: &'static str,
    pub primary_pattern: &'static str,
    pub secondary_mode: &'static str,
    pub expected_edge_timebase: &'static str,
    pub edges: Vec<SyntheticTriggerEdge>,
}

pub fn m50_dry_crank_fixture_placeholder() -> M50DryCrankFixturePlaceholder {
    M50DryCrankFixturePlaceholder {
        format_version: M50_DRY_CRANK_FIXTURE_FORMAT_VERSION,
...
impl TriggerReplayFrame {
    /// Public contract for viewer exports built from replay frames.
    pub const VIEWER_SCHEMA_VERSION: u8 = 1;
    /// Stable field order for future tooth/composite log viewers.
    pub const VIEWER_FIELD_NAMES: [&'static str; 6] = [
        "index",
        "edge",
        "decoder_event",
        "sync_loss",
        ...
        match edge.channel {
            ...
            ReplayTriggerChannel::Cam => match decoder_config.secondary.mode {
                ...
                | SecondaryTriggerMode::OemPattern { .. } => {
                    unsupported_cam_edge = true;
                    unsupported_cam_edges = unsupported_cam_edges.saturating_add(1);
                    if let Err(reason) = decoder.ingest_secondary_edge(trigger_edge(edge.kind)) {
                        sync_loss = Some(reason);
                    }
                }
...
pub fn replay_authority_has_720_phase(authority: EngineTimeAuthority) -> bool {
    matches!(
        authority.phase,
