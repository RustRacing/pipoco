...
| `tests/fm0016_board_adapter_contract.rs` | 10/10 PASS |
...
| `tests/fm0016_board_adapter_contract.rs` | 16/16 PASS |

### v5 Key Changes

- **No blocked rows**: All conformance map rows are `covered` or `adapter-contract`. No row uses `status=blocked`.
- **Adapter contracts**: Typed Rust enums (`RuntimeAdapterContract`, `SchedulerAdapterContract`, `CoreAdapterContract`, `BoardAdapterContract`) document every model gap between frozen spec and product implementation.
...
- **No oracle-copy surfaces**: Scheduler has no `DifferentialOutputSurface::from_spec`. Runtime/r00t use product APIs only for observed values.
- **ECU-spec dependency hygiene**: No production code in `ecu-runtime`, `ecu-scheduler`, `ecu-core`, `ecu-target-common`, or board crates depends on `ecu-spec`.
