...
use crate::outputs::{ScheduledActionExecutor, ScheduledOutputsLike, TransitionApplyError};
use ecu_domain::Micros;
use ecu_io::{CalibrationStore, CaptureSink, SensorSource, TransportPublisher, Watchdog};
...
pub fn run_split_scheduled_tick<S, C, W, T, P, O, const Q: usize, const D: usize>(
...
mod tests {
    ...
    use crate::outputs::{ScheduledOutputPin, ScheduledOutputs4};
    use ecu_calibration::PersistedCalibrationBlob;
    use ecu_domain::{Degrees10, Kpa10, Lambda100, PulseWidthUs, Rpm};
    use ecu_io::{ActionExecutor, CaptureSample};
    use ecu_runtime::{
        Action, BaseFuelModel, EnrichmentInputs, IgnitionInputs, LambdaTrimInputs, TorqueInputs,
    };
    ...
    #[test]
    fn run_split_scheduled_tick_queues_drains_and_applies_outputs() {
        ...
        let result = run_split_scheduled_tick(
            &mut adapter,
            Micros::new(10_000),
    ...
    #[test]
    fn run_split_scheduled_tick_returns_zero_counts_when_unsynced() {
        let mut adapter = adapter();
    ...
    #[test]
    fn run_split_scheduled_tick_clears_queue_after_sync_loss() {
        let mut adapter = adapter();
        sync_adapter(&mut adapter);
        adapter
    ...
    #[test]
    fn run_split_scheduled_tick_reports_apply_error_without_partial_pin_writes() {
        let mut adapter = adapter();
