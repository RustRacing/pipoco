...
//! the ecu_core::persist::KvStore interface.
...
impl RpFlash {
    ...
    fn abs(off: u32) -> u32 { Self::BASE + off }

    fn read_block(addr: u32, out: &mut [u8]) {
        let src = (0x1000_0000 + addr) as *const u8;
        for i in 0..out.len() { unsafe { out[i] = core::ptr::read_volatile(src.add(i)); } }
    }
...
impl NorFlash for RpFlash {
    ...
    fn write(&mut self, offset: u32, bytes: &[u8]) -> Result<(), Self::Error> {
        embedded_storage::nor_flash::check_write(self, offset, bytes.len()).map_err(|_| FlashErr)?;
        ...
        while remaining > 0 {
            ...
            let page_end_off = page_start_off + ERASE_SIZE;
            ...
            // Erase + Program
            unsafe {
                Self::rom_erase(Self::abs(page_start_off as u32), ERASE_SIZE);
                Self::rom_program(Self::abs(page_start_off as u32), &page);
            }
...
#[derive(Debug, PartialEq, Eq)]
enum TableItemErr { BufferTooSmall, BadHeader }
impl StorageItemError for TableItemErr {
    fn is_buffer_too_small(&self) -> bool { matches!(self, TableItemErr::BufferTooSmall) }
}
...
impl StorageItem for TableItem {
    ...
    fn serialize_into(&self, buf: &mut [u8]) -> Result<usize, Self::Error> {
        ...
        const HDR: usize = 8;
        if buf.len() < HDR + 512 { return Err(TableItemErr::BufferTooSmall); }
        buf[0] = 0x49; buf[1] = 0x50; // 'IP'
        buf[2] = self.key;
        buf[3] = 0x00; buf[4] = 0x02; // len = 512 (LE)
        buf[5] = 0x00;
        buf[HDR..HDR + 512].copy_from_slice(&self.payload);
        let crc = crc16_ccitt(0xFFFF, &buf[HDR..HDR + 512]);
        buf[6] = (crc & 0xFF) as u8;
        buf[7] = (crc >> 8) as u8;
        Ok(HDR + 512)
    }

    fn deserialize_from(buf: &[u8]) -> Result<(Self, usize), Self::Error> where Self: Sized {
        const HDR: usize = 8;
        if buf.len() < HDR { return Err(TableItemErr::BufferTooSmall); }
        if buf[0] != 0x49 || buf[1] != 0x50 { return Err(TableItemErr::BadHeader); }
        let key = buf[2];
        let len = (buf[4] as usize) << 8 | (buf[3] as usize);
        if len != 512 || buf.len() < HDR + 512 { return Err(TableItemErr::BufferTooSmall); }
        let mut payload = [0u8; 512];
        payload.copy_from_slice(&buf[HDR..HDR + 512]);
        let crc = ((buf[7] as u16) << 8) | (buf[6] as u16);
        if crc16_ccitt(0xFFFF, &payload) != crc { return Err(TableItemErr::BadHeader); }
...
impl SeqKv { pub const fn new() -> Self { Self } }

impl KvStore for SeqKv {
    fn read(&mut self, key: &[u8], out: &mut [u8]) -> Result<usize, KvError> {
        let k = match key { b"fuel" => KEY_FUEL, b"ign" => KEY_IGN, _ => return Err(KvError::NotFound) };
        let mut flash = RpFlash;
        match map::fetch_item::<TableItem, _>(&mut flash, REGION_OFFSET..(REGION_OFFSET + REGION_SIZE as u32), k) {
            Ok(Some(item)) => { out[..512].copy_from_slice(&item.payload); Ok(512) }
    ...
    fn write(&mut self, key: &[u8], data: &[u8]) -> Result<(), KvError> {
        ...
        let k = match key { b"fuel" => KEY_FUEL, b"ign" => KEY_IGN, _ => return Err(KvError::NotFound) };
        let mut pl = [0u8; 512]; pl.copy_from_slice(data);
        let item = TableItem { key: k, payload: pl };
        let mut flash = RpFlash;
        map::store_item::<TableItem, _, {ERASE_SIZE}>(
            &mut flash,
            REGION_OFFSET..(REGION_OFFSET + REGION_SIZE as u32),
            item,
